...
program
  ...
  .description(
    "Add a win to a section (delivered, collaboration, growth, impact, feedback, goals)"
  )
  .action((section?: string, text?: string) => {
    if (!section || !text) {
      addInteractive();
    } else {
      add(section, text);
...
program
  ...
  .action((options: { name?: string; role?: string; period?: string }) => {
    config(options);
  });
