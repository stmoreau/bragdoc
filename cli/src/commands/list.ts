...
import { findSection, SECTIONS, type SectionKey } from "../sections.js";

export function list(section?: string): void {
  if (!bragDocExists()) {
  ...
  if (doc.name || doc.role || doc.period) {
    if (doc.name) console.log(chalk.bold(doc.name));
    if (doc.role) console.log(chalk.dim(doc.role));
    if (doc.period) console.log(chalk.dim(doc.period));
    console.log();
  }
